...
public class Ch3Ex2Activity extends AppCompatActivity {
    ...
    private void resetTargetAnimation() {
        ...
        scaleX_animator.setRepeatMode(ValueAnimator.REVERSE);
        scaleX_animator.setDuration(Integer.parseInt((durationSelector.getText().toString())));

        ObjectAnimator scaleY_animator = ObjectAnimator.ofFloat(target, "scaleY", 1, 2);
        scaleY_animator.setRepeatCount(ValueAnimator.INFINITE);
        scaleY_animator.setRepeatMode(ValueAnimator.REVERSE);
        scaleY_animator.setDuration(Integer.parseInt((durationSelector.getText().toString())));

        // TODO ex2-2：在这里实现另一个 ObjectAnimator，对 target 控件的透明度进行修改，从 1 到 0.5f 循环
        ObjectAnimator alpha_animator = ObjectAnimator.ofFloat(target,"alpha",1f, (float)0.5);
        alpha_animator.setRepeatCount(ValueAnimator.INFINITE);
        alpha_animator.setRepeatMode(ValueAnimator.REVERSE);
        alpha_animator.setDuration(Integer.parseInt((durationSelector.getText().toString())));
