...
public class MainActivity extends AppCompatActivity {
    ...
    private void getData(String studentId){
        new Thread(new Runnable() {
            @Override
            public void run() {
                MessageListResponse msgs = baseGETRepoFromRemote(studentId,
                        "U0pUVS1ieXRlZGFuY2UtYW5kcm9pZA==");
                if (msgs != null && msgs.success){
                    new Handler(getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            adapter.setData(msgs.feeds);
                            Log.d(TAG, String.format("%b", msgs.success));
                        }
                    });
                }
            }
...
//                String.format("https://api-sjtu-camp-2021.bytedance.com/homework/invoke/messages?student_id=%s", "519070910059");
        ...
        try{
            ...
            Log.d(TAG, String.format("%d", conn.getResponseCode()));

            if (conn.getResponseCode() == 200){
                Log.d(TAG, "baseGETRepoFromRemote: here");
